...
ModuleReferencesStream.prototype._parseTodo = function(row)
{
        ...
        if (row.file && path.extname(row.file) !== '.js')
        {
                this.emit('ignoreInput', row);
                return q();
        }
        ...
        {
                myErr = Error('Error parsing source file "' + row.file + '" : ' + err.message + ' ('+err.lineNumber+':'+err.column+':'+err.index+')');
                myErr.cause = err;
                throw myErr;
        }
